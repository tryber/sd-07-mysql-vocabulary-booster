...
(SELECT ROUND(AVG(SALARY), 2) FROM hr.employees AS E WHERE E.JOB_ID = J.JOB_ID) AS "Média Salarial",
(SELECT
CASE
...
WHEN (ROUND(AVG(SALARY), 2) >= 7501 AND ROUND(AVG(SALARY), 2) <= 10500) THEN 'Sénior'
